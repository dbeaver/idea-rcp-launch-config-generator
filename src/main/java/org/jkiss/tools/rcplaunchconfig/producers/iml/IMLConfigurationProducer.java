    ...
    private String generateIMLBundleConfig(@NotNull BundleInfo bundleInfo, @NotNull Result result) throws IOException {
        ...
        if (!outputs.isEmpty()) {
            builder.append("  <output url=\"")
                .append(getFormattedRelativePath(bundleInfo.getPath().resolve(outputs.getFirst()), false, false))
                .append("\"/>").append("\n");
        }
        ...
        boolean hasContent = !sources.isEmpty() && !sources.getFirst().isEmpty();
        ...
        for (Pair<String, VersionRange> importPackage : bundleInfo.getImportPackages()) {
            if (bundlePackageImports.get(importPackage) != null) {
                if (importPackage.getSecond() != null) {
                    ...
                    if (!alreadyResolved) {
                        BundleInfo info = bundleInfos.getFirst().getFirst();
                        appendBundleInfo(
                            bundleInfo,
